...
class Order {
    ...
    public function cancel_order($order_id) {
        ...
        try {
            ...
            if ($order['order_status'] == 2) {
                ...
                //订单小金库消费金额增加到用户小金库
                Db::name('member')->where(['uid'=>$order['uid']])->setInc('secondAccount',$order['secondPay']);
    ...
    public function order_info($order_id, $uid = null) {
        ...
        $order = Db::name('order')->alias('o')->field('o.*,m.openId,m.uname,m.uwecat,d.dispatch_title')
    ...
    public function order_list($params) {
        ...
        if ($params['isHistory'] == 1) {
            ...
            $list = Db::name('Order')->alias('Order')->field('Order.*,d.dispatch_title,m.uname,m.uwecat')
            ...
            //分页
            $datas = Db::name('Order')->alias('Order')->field('Order.*,d.dispatch_title,m.uname,m.uwecat')
                    ->join('__DISPATCH__ d', 'Order.dispatch_id=d.id')
                    ->join('__MEMBER__ m', 'Order.uid=m.uid')
                    ->where($map)
    ...
    public function update_order($order_id) {
        ...
        foreach ($list as $k => $v) {
            ...
            if ($v['order_option_id']) {
                ...
                //需要扣减库存
                if ($v['subtract']) Db::name('goods')->where('goods_id', $v['goods_id'])->setDec('quantity', $v['goods_quantity']);
            }
        }
